...
exports.getSummonerByName = ({ name, region }) => {
  const url =
    `https://${region}.api.riotgames.com/api/lol/${region}/v1.4/summoner/` +
...
exports.getStatsSummary = ({ summonerId, region }) => {
  const url =
    `https://${region}.api.riotgames.com/api/lol/${region}/v1.3/stats/` +
...
exports.getMatchListBySummonerId = ({ summonerId, region }) => {
  ...
  const url =
    `https://${region}.api.riotgames.com/api/lol/${region}/v2.2/matchlist/` +
...
exports.getMatchDetailsByMatchId = ({ matchId, region }) => {
  const url =
    `https://${region}.api.riotgames.com/api/lol/${region}/v2.2/match/${matchId}` +
    `?api_key=${apiKey}`
...
exports.getLeagueBySummonerId = ({ summonerId, region }) => {
  const url =
    `https://${region}.api.riotgames.com/api/lol/${region}/v2.5/league/` +
