...
function cacheSummonerObject ({ name, region, cacheKey }) {
  ...
  return api.getSummonerByName({ name, region })
    .then((data) => {
      // Store the original data for use by the marshaller
      summonerData = data
      return marshaller.marshallSummonerData(summonerData)
    })
    .then((marshalledData) => {
      // Store the newly marshalled data
      summonerData = marshalledData
